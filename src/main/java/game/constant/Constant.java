package game.constant;

/**
 * Created by pengyi
 * Date : 17-9-6.
 * desc:
 */
public class Constant {

    public static int appVersion = 4;
    public static String apiUrl = "http://127.0.0.1:49999/api";
    public static String userInfoUrl = "/user/info";
    public static String userListUrl = "/user/list";
    public static String userRankingList = "/user/ranking";
    public static String gamerecordCreateUrl = "/gamerecord/create";
    public static String wechatLogin = "/user/login_wechat";
    public static String moneyDetailedCreate = "/money_detailed/create";
    public static String share = "/user/share";
    public static String arenaListUrl = "/arena/list";
    public static String arenaInfoUrl = "/arena/info/";
    public static String gamerecordListUrl = "/gamerecord/list";
    public static String gamerecordInfoUrl = "/gamerecord/info/";
    public static String exchangeListUrl = "/exchange/list/";
    //    public static String gameServerIp = "119.23.67.179";
    public static String gameServerIp = "119.23.60.87";
    //    public static String gameServerIp = "192.168.1.99";
    public static String updateInfoUrl = "/user/updateInfo";

    public static int messageTimeout = 60000;
}
